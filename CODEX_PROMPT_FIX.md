...
**原因**: 旧CSS `style-main.css` の `#searchTOP .rentcheck span`（`width:31%; font-size:24px; margin...`）が、theme-v3 の `.rentcheck label span` より**セレクタ詳細度で勝っている**。`#searchTOP` 自体の `background:#f0f0f0; padding:40px 30px` も未打ち消し。チェック時の✓も旧CSS `#searchTOP .rentcheck input:checked + span::before {display:none}` に潰されている。
...
**原因**: 旧CSSがliの幅を固定しており、theme-v3 の `.after_box ul li`（inline-flex）が幅を上書きできていない。
**修正**: theme-v3 に追加:
...
**原因と注意**: 見出しから「Qn.」を除去する処理を `updateDiagnosisState` に足すだけでは動かない。**設問リスト `ul.select_box li` のクリックハンドラが `return false` で伝播を止める**ため、document委譲のリスナーが発火しないケースがある（検証で確認済み）。
...
2. **エントリーモーダルの質問二重**（index.html）: v3ヘッダー「カードローンの利用経験を教えてください」と旧文言「Q.カードローンの利用は」が両方表示されている。`<p> Q.カードローンの利用は </p>` の行を削除（クラス無しのpで、JS依存なし。`.select_modal_btn1/2` は触らない）。
