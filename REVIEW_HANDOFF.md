...
| LPO2 | 未コミット | ファーストビューを入口ポップアップ中心にし、女性ナビゲーターの会話演出と短い思考余白を追加 |
...
| index.html 女性ナビ案内 | こんにちは。カードローン診断ナビゲーターです。 / まずは現在のご状況を教えてください。 |
| index.html ローディング | はじめての方向けの質問を準備しています / 見やすい順番に整理しています / まもなく診断を開始します / ご経験に合わせた質問を準備しています / 比較しやすい条件を整理しています |
| 診断モーダル 女性ナビ | いくつか質問します。近いものを選んでください。 / いただいた回答を確認しながら、次の質問へ進みます。 |
...
- GitHub PagesのJSキャッシュ対策として、`v3-accessibility-cvr.js` の読み込みクエリを `pages-demo-4` に更新。
...
- 版構成を整理。アコム先頭版は `result.html` / `beginner_result.html`、モビット先頭版は `mobit.html` / `mobit_result.html` / `mobit_beginner.html` / `mobit_beginner_result.html`、詳細口コミカードV2は `result_v2.html` / `beginner_result_v2.html` として確認できる状態にした。
- `demo-list.html` の分類を実際の表示順に合わせ、「共通TOP」と「アコム先頭版」「モビット先頭版」を分離した。
- V2のCSS/JS読込は `result-cards-v2.css?v=38` / `result-cards-v2.js?v=31` で経験者・初心者を統一している。
- 公式バナー、PR表記、注釈、redirect導線、診断の固定ID/name/クラスは監査対象として保全を確認した。
- 未追跡の `images/review-people-lineup.png` は一覧・ページから参照されていないため、公開対象から除外した。
